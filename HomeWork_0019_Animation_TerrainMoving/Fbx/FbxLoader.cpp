	...
	for (FbxMeshData* meshData : meshDatas)
	{
		...
		for (int i = 0; i < deformerCount; i++)
		{
			...
			for (int clusterIndex = 0; clusterIndex < skin->GetClusterCount(); clusterIndex++)
			{
				...
				// 해당 관절이 가지고 있는 컨트롤 포인트만큼, 관절쪽 피부를 매꿔줘야되니깐
		 ...
		 // 가중지 전체의 합은 1, xyzw, 그래서 가중치를 노멀라이즈
...
void FbxLoader::ReadAnimationData(wstring name)
{
	...
	FbxLongLong start = timeSpan.GetStart().GetFrameCount(timeMode); // 시작시간이 0이 아닐수도 있음
	...
	clipData->FrameCount = (int)(stop - start + 1); // 전체시간
