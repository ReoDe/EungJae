	...
	, Go(0), Back(0)
{
	...
	D3DXMatrixIdentity(&Ry); // 회전초기화
	D3DXMatrixIdentity(&Rx); // 회전초기화
	D3DXMatrixIdentity(&Rz); // 회전초기화
...
void DrawGrid::Update()
{
	...
	// 포지션을 그쪽 방향으로 천천히 이동
	// x축 y축 비율 가져와서 이동하게.
	if (CubePosition.x - CubePositionDirection.x > 0.001f || 
		CubePosition.x - CubePositionDirection.x < -0.001f)
		MovePoint(&CubePosition, &CubePositionDirection, 50, &Ry, &Rx, &Rz);
				// 현재 포지션,		마우스클릭포지션,    속도
	...
	// 이렇게 하면 포지션.x를 받지 못함
	//D3DXMatrixTranslation(&Tk, Go, 1, 1);
...
void DrawGrid::Render()
{
	...
	D3DXMatrixTranslation(&T, CubePosition.x, GetPickingY() + 0.5f, CubePosition.z);

	worldBuffer->SetMatrix(Ry * Rx * Rz * T );
...
void DrawGrid::MovePoint(D3DXVECTOR3*Origin, D3DXVECTOR3* Direction, float Speed, D3DXMATRIX* Ry, D3DXMATRIX* Rx, D3DXMATRIX* Rz)
{
	...
	float angle = 0;
	angle = atanf(Normal.x / Normal.z);

	D3DXMatrixRotationY(&(*Ry), angle);

	// xz평면에서 기울기

	// 물체중점이 어느 삼각형에 있는지 확인해야됨.
	...
	if (ddx + ddz <= 1) // 안쪽삼각형
	{
		...
		// z축 회전
		angle = atanf(temp.x / temp.y) - Math::PI / 2;
		D3DXMatrixRotationX(&(*Rx), angle);

		// x축 회전
		angle = atanf(temp.z / temp.y) - Math::PI / 2;
