...
void D3D::Clear(D3DXCOLOR color, ID3D11RenderTargetView* rtv, ID3D11DepthStencilView* dsv)
{
	...
	deviceContext->ClearRenderTargetView(rtv, color); // 우리들이 그릴 화면 (api 의 hdc) 어딘가에 찍어줄화면  을 지움
	deviceContext->ClearDepthStencilView(dsv, D3D11_CLEAR_DEPTH, 1, 0);
}

void D3D::Present()
{
	swapChain->Present(d3dDesc.bVsync == true ? 1 : 0, 0); // 더블 버퍼링을 위해서, 틱당 랜더링 속도를 맞춰주기위해서 백버퍼를 출력하고 그 위헤 전면버퍼를 덧씌우는것
							// 수직 동기화
}
