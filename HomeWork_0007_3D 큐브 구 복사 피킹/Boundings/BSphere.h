...
struct BSphere : public ILine
{
	...
	void GetLine(D3DXMATRIX& world, vector<D3DXVECTOR3>& lines)
	{
		...
		for (UINT i = 0; i <= StackCount; i++)
		{
			...
			if (i == 0 || i == StackCount)
			{
				vertices[index] = Center + D3DXVECTOR3(0, i == 0 ? sphere.Radius : -sphere.Radius, 0);
			...
			else
			{
				...
				for (UINT k = 0; k <= SliceCount; k++)
				{
					...
					vertices[index] = Center + D3DXVECTOR3(x, y, z);
					index++;
				}
			}//if(i)
		}//for(i)
		...
		/*if (vertices.size() != 0)
		{
			...
			{
				D3DXVec3TransformCoord(&vertices[i], &vertices[i], &world);
			}
...
*/
	...
	BSphere Transform(D3DXMATRIX& world)
	{
		...
		//D3DXVec3TransformCoord(&sphere.Center, &sphere.Center, &world);


		//sphere.Center = D3DXVECTOR3(world._41, world._42, world._43);
