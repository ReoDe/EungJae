	...
	while (true)
	{
		...
		else
		{
			...
			D3D::Get()->Clear();
			{
				...
				{
					// 혹시나 클리어하게되면 3D화면이 날라가니깐 여기서 하는거아님,
		...
		ImGui::Invalidate();
		{
			if (program != NULL)
			{
				...
				if (DirectWrite::Get() != NULL)
				{
					...
					//DirectWrite::DeleteBitMap();
				}
				

				if (D3D::Get() != NULL)
					D3D::Get()->ResizeScreen(width, height); // 사이즈 변경함수

				if (DirectWrite::Get() != NULL)
				{
					DirectWrite::CreateSurface();
					//DirectWrite::CreateBitMap();
				}
