...
Terrain::Terrain(ExecuteValues * values, Material * material, wstring heightMap)
	: values(values), material(material), capacity(10.0f), BrushType(0), TreeNumber(3)
{
...
void Terrain::Update()
{
	...
	if (D3DXVec3Length(&brushBuffer->Data.Location) > 0.0f)
	{
		...
		else if (BrushType == 2) // 나무심기
		{
			if (type == Rect && Mouse::Get()->Down(0))
			{
				PlusTreeRect(brushBuffer->Data.Location, TreeNumber);
			}

			if (type == Circle && Mouse::Get()->Down(0))
			{
				PlusTreeCircle(brushBuffer->Data.Location, TreeNumber);
			}
			if (type == Cone && Mouse::Get()->Down(0))
			{
				PlusTreeCircle(brushBuffer->Data.Location, TreeNumber);
			}
...
void Terrain::Render()
{
	...
	ImGui::RadioButton("tarrian brush", &BrushType, 1); ImGui::SameLine(240);
	ImGui::RadioButton("Plus Tree", &BrushType, 2);
	...
	{
		ImGui::SliderInt("Tree Number", (int*)&TreeNumber, 0, 10);
	}
	...
	ImGui::Separator();
	ImGui::Text("test");
	int vectersize = billboards.size();
	ImGui::SliderInt("tree vector size", &vectersize, 0, 100);
...
void Terrain::AdjustHeight(D3DXVECTOR3 & location)
{
	...
	(
		vertexBuffer, 0, NULL, &vertices[0], sizeof(VertexTextureNormal), vertexCount
	);
...
void Terrain::PlusTreeCircle(D3DXVECTOR3 & location, UINT treeNumber)
{

	UINT size = (UINT)brushBuffer->Data.Range; // range는 0,1,2 ...

	float Radius = (float)size / 2;


	int TreeNumber = size * treeNumber;
	...
	for (int i = 0; i < TreeNumber; i++)
	{
		...
		int Radius100 = Radius * 100;
		int randRadius100 = rand() % Radius100;

		float randRadius = randRadius100 / 100.000f;
		float randAngle = rand() % 360;


		D3DXVECTOR3 position;
		position.x = location.x + randRadius * cosf(randAngle);
		position.z = location.z + randRadius * sinf(randAngle);
		position.y = GetHeight(position.x, position.z);
		position.y += scale.y * 0.5f;
...
void Terrain::PlusTreeRect(D3DXVECTOR3 & location, UINT treeNumber)
{
	UINT size = (UINT)brushBuffer->Data.Range; // range는 0,1,2 ...
...
void Terrain::MovePoint(D3DXVECTOR3*Origin, D3DXVECTOR3* Direction, float Speed, D3DXVECTOR3& Rotation)
{
	...
	Rotation.y = (float)(Angle - Math::PI * 0.5);
